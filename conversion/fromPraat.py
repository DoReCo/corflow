...
Note: The encoding is automatically detected using the first 3 bytes of a file,
      ...
      dictionary.
Note: 'latin_1' and 'utf-8' are discriminated by the presence of 'Ã' in
      the transcription. 
      'latin_1' will therefore require reading the whole file once.
...
def _loadLong(path,trans,enc):
    ...
    def _longText(f):
        ...
        text = f.readline().split('"',1)[1]
        return _escape(text.rsplit('"',1)[0])
